...
const exposedHeaders = [
  "inventory2-access-token",
  "inventory2-refresh-token",
  "set-cookie",
];
if (!corsOrigin) {
  app.use(
...
const {
  ...
  internalTransferLineRouter,
  internalTransferLogRouter,
  transferFlowShareTokenRouter,
} = require("restLayer");

app.use("", internalTransferRouter);
app.use("", internalTransferLineRouter);
app.use("", internalTransferLogRouter);
app.use("", transferFlowShareTokenRouter);
...
app.get("/getPostmanCollection", (req, res) => {
  const filePath = path.join(
    __dirname,
    "inventory-transferFlow-postman-collection.json",
  );
  console.log("Downloading Postman Collection from:", filePath);
  res.setHeader("Content-Type", "application/json");
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="inventory-transferFlow-postman-collection.json"',
  );
