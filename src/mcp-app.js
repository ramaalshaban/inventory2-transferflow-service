...
app.post("/mcp", async (req, res) => {
  ...
  if (sessionIdHeader && sessions[sessionIdHeader]) {
    ...
    const {
      ...
      internalTransferMcpRouter,
      internalTransferLineMcpRouter,
      internalTransferLogMcpRouter,
      transferFlowShareTokenMcpRouter,
      getSessionRouter,
    } = require("mcpLayer")(initialHeaders);

    const sessionRouter = getSessionRouter(initialHeaders);

    internalTransferMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
    ...
    internalTransferLineMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
    ...
    internalTransferLogMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
    ...
    transferFlowShareTokenMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
