...
console.log("Starting inventory-transferflow-service service...");
...
const start = async () => {
  process.title = "node-inventory2-transferflow-service";
  ...
  try {
    ...
    await setRedisData(
      "inventory2-transferflow-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key inventory2-transferflow-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "inventory-transferflow-service is listening HTTP/REST port " +
...
/* starting the server inventory-transferflow-service .... */
start();
