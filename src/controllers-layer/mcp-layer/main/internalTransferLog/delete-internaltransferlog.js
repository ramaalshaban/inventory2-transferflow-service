...
class DeleteInternalTransferLogMcpController extends TransferFlowMcpController {
  constructor(params) {
    super("deleteInternalTransferLog", "deleteinternaltransferlog", params);
    this.dataName = "internalTransferLog";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteInternalTransferLogManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "deleteInternalTransferLog",
    description:
      "Soft-delete a transfer log if needed (should almost never be needed).",
    parameters: DeleteInternalTransferLogMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteInternalTransferLogMcpController =
        new DeleteInternalTransferLogMcpController(mcpParams);
      try {
        const result =
          await deleteInternalTransferLogMcpController.processRequest();
        //return DeleteInternalTransferLogMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
