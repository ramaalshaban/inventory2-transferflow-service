...
const updateInternalTransferLogById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const [rowsCount, [dbDoc]] = await InternalTransferLog.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateInternalTransferLogById;
