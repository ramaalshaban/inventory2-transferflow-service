...
const getInternalTransferLogStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(
          InternalTransferLog.count({ where: queryWithSoftDelete }),
        );
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            InternalTransferLog.sum(statParts[1], {
          ...
          promises.push(
            InternalTransferLog.avg(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            InternalTransferLog.min(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            InternalTransferLog.max(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
    ...
    if (promises.length == 0) {
      return await InternalTransferLog.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingInternalTransferLogStatsByQuery",
      err,
    );
  }
};

module.exports = getInternalTransferLogStatsByQuery;
