...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("transferFlowShareToken");
  await elasticIndexer.indexData(data);
};

const updateTransferFlowShareTokenById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await TransferFlowShareToken.findOne({
    ...
    const [rowsCount, [dbDoc]] = await TransferFlowShareToken.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateTransferFlowShareTokenById;
