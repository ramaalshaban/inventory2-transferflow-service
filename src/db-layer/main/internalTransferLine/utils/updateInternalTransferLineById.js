...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("internalTransferLine");
  await elasticIndexer.indexData(data);
};

const updateInternalTransferLineById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await InternalTransferLine.findOne({
    ...
    const [rowsCount, [dbDoc]] = await InternalTransferLine.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateInternalTransferLineById;
