...
describe("GetInternalTransferByStatus", () => {
  let ControllerClass;
  let req;
  ...
  beforeEach(() => {
    ...
    ControllerClass = proxyquire(
      "../../../../src/manager-layer/main/InternalTransfer/get-internaltransferbystatus",
      {
        "./InternalTransferManager": class {
  ...
  it("should initialize controller with correct base properties", () => {
    ...
    expect(instance.options.name).to.equal("getInternalTransferByStatus");
    expect(instance.options.controllerType).to.equal("rest");
    expect(instance.options.crudType).to.equal("get");
    expect(instance.dataName).to.equal("internalTransfer");
  });
});
